...
//        include("Link/languge.php"); 
        ...
        if(isset($_POST['submit']))
        {
            ...
            $sql="UPDATE `tblsliderservices` SET `Icon`=:ICON,`name`=:NA,`about`=:ABU,`buttonurl`=:URL,`Status`=:SS WHERE id=:sliid";
...
<head>
  ...
  <title> <?php echo $CNAME." || "."   تحديث العارض الفرعي  " ?> </title>
  <!-- Favicons -->
...
<?php 
$id=intval($_GET['sliid']);
$sql = "SELECT * FROM tblsliderservices WHERE id=:sliid";
$query = $dbh->prepare($sql);
$query->bindParam(':sliid',$id,PDO::PARAM_INT);
$query->execute();
...
?>  
                ...
                <div class="row mb-3">
                  ...
                  <div class="col-sm-10">
                    <input type="text" class="form-control" name="ABU" value="<?php echo htmlentities($result->about); ?>" required>
                  </div>
                </div>
                <div class="row mb-3">
                  <label for="inputText" class="col-sm-2 col-form-label"> <?php echo "رابط تجريب النسخة"; ?></label>
                  <div class="col-sm-10">
                    <input type="text" class="form-control" name="URL" value="<?php echo htmlentities($result->buttonurl); ?>" >
                  </div>
                </div>
               <div class="row mb-3">
                  <label for="inputNumber" class="col-sm-2 col-form-label"><?php echo "أيقونة الملف"; ?> </label>
                  <div class="col-sm-10">
                    <input class="form-control" type="text" id="formFile" name="ICON" value="<?php echo htmlentities($result->Icon); ?>">
                  </div>
                </div>    
