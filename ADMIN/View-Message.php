...
<?php echo "إدارة الاتصالات";?> 
  <meta content="" name="description">
  <meta content="" name="keywords">
  <title> <?php echo $CNAME." || "."    إدارة الاتصالات - معاينة الرسالة  " ?> </title>
  <!-- Favicons -->
...
$sql = "UPDATE tblcontactdata SET Is_Read=1 WHERE id=:view_m";   
$query = $dbh -> prepare($sql);
$query-> bindParam(':view_m',$id,PDO::PARAM_INT);
$query->execute();
        
        
$sql = "SELECT * FROM tblcontactdata WHERE id=:view_m";
$query = $dbh -> prepare($sql);
$query-> bindParam(':view_m',$id,PDO::PARAM_INT);
$query->execute();
...
foreach($results as $result){
             ...
             <ul class="px-0 list-unstyled" align="right">
                  <li><b>إسم المرسل : </b><?php echo htmlentities($result->FullName);?></li>
                  <li><b>عنوان المرسل : </b> <?php echo htmlentities($result->UserIp);?></li>
                  <li><b>هاتف المرسل : </b><?php echo htmlentities($result->PhoneNumber);?></li>
                  <li><b>إيميل المرسل : </b><?php echo htmlentities($result->EmailId);?></li>
                  <li><b>تاريخ الارسال : </b><?php echo htmlentities($result->PostingDate);?></li>
              </ul><hr>
              <table class="table ">
                    <tbody>
                      <tr>
                        <th scope="row">عنوان الرسالة</th>
                        <td><p><?php echo htmlentities($result->Subject);?></p>
                        </td>
                      </tr>
                     <tr>
                        <th scope="row">نص الرسالة </th>
                        <td><p style="color:blue;"><?php echo htmlentities($result->Message);?></p>
                        </td>
                      </tr>
                    </tbody>
                  </table>
