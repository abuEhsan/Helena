...
//        include("Link/languge.php"); 
        ...
        if(isset($_POST['submit']))
        {
            ...
            $sql="UPDATE `tblsliderown` SET `Icon`=:ICON,`name`=:NA,`about`=:ABU,`buttonurl`=:URL,`Status`=:SS WHERE id=:sliid";
...
<head>
  ...
  <title> <?php echo $CNAME." || "."   تحديث العارض لبرئيسي  " ?> </title>
  <!-- Favicons -->
...
<?php 
$id=intval($_GET['sliid']);
$sql = "SELECT * FROM tblsliderown WHERE id=:sliid";
$query = $dbh->prepare($sql);
$query->bindParam(':sliid',$id,PDO::PARAM_INT);
$query->execute();
