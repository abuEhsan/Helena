...
if(!empty($_POST["classid"])) 
{
 ...
 if(!is_numeric($cid)){
 	...
 	echo htmlentities("اتفاصيل القسم");exit;
 }
 else{
...
if(!empty($_POST["classid1"])) 
{
 $cid1=intval($_POST['classid1']);
     if(!is_numeric($cid1)){
      ...
      echo htmlentities("القسم غير صحيح");exit;
     }else{
         $status=0;	
         ...
         while($row=$stmt->fetch(PDO::FETCH_ASSOC))
         {?>
          <p> <?php echo htmlentities($row['SubjectName']); ?><input type="text"  name="marks[]" value="" class="form-control" required="" placeholder="ادخل النتيجة من 100" autocomplete="off"></p>
...
<p>
<?php
echo "<span style='color:red'> لقد تم عرض نتيجة هذا الطالب فعلا .</span>";
