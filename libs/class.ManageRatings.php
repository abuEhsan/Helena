<?php
	class ManageRatings{
		...
		protected $db_name = 'market_helena';
		...
		function __construct(){
			...
			if(isset($id)){
				$query = $this->link->query("SELECT * FROM tblproducts WHERE id = '$id'");
			}
			else
			{
				$query = $this->link->query("SELECT * FROM tblproducts");
			}
		...
		function insertRatings($id,$rating,$total_rating,$total_rates,$ip_address){
			$query = $this->link->query("UPDATE tblproducts
			SET rating = '$rating',
